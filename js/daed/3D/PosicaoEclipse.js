ON_DAED['3D'].PosicaoEclipse = function (observacaoFirmamento, observacaoLua) {
    ...
    var terraRef = new THREE.Mesh(
            new THREE.SphereGeometry(observacaoFirmamento.getRaioHorizonte() / 27, 64, 32),
            new THREE.MeshLambertMaterial({
                map: new THREE.ImageUtils.loadTexture('imgs/texturas/terra/bandeiras.jpg')
            })
    );
    ...
    ON_DAED['3D'].register(scene, terra, function() {
        ...
        terraRef.rotation.y = hora * Math.PI * 2 - Math.PI;
    });
...
//        camera.updateProjectionMatrix();
    ...
    o.requestEclipseSolar = function () {
        ...
        var currentEclipse = ON_DAED.ASTRO.getSolarEclipse(observacaoFirmamento.getDate());
        var add = 0;
        if (!currentEclipse.noEclipse) { /* Possibilita clicar no botao de eclipse solar para calcular o proximo Eclipse. Sem isso o programa trava no Eclipse inicial */
            add = 15;
        }
        ON_DAED["3D"].workerLogica.postMessage({'cmd': 'eclipseSolar', 'data': observacaoFirmamento.getDate() + add});
    };
